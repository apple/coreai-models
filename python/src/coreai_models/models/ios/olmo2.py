...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # Shape is (batch, channels, 1, seq) after Conv2d — norm over channels
        query = query.transpose(-3, -1)  # (batch, seq, 1, channels)
        key = key.transpose(-3, -1)
        query = self.q_norm(query).transpose(-3, -1)  # back to (batch, channels, 1, seq)
        key = self.k_norm(key).transpose(-3, -1)

        # Reshape into heads for RoPE
        query = (
            query.transpose(-3, -1)
...
class Olmo2Extend(nn.Module):
    ...
    def forward(
        ...
        if self.lm_head is not None:
            return self.lm_head(out.squeeze(2))
