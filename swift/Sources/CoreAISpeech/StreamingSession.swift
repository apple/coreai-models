...
package struct EndpointDetector {
    ...
    ///
    /// Two ways to fire, and both land on a pause so the transducer can always be reset
    /// safely:
    ///
    ...
    ///
    /// `framesAdvanced` must be *duration-weighted*, not a step count. A blank with
    /// duration 4 skips 320 ms of audio in a single step, so counting steps would
    /// under-measure silence by up to 4×.
    package mutating func observe(framesAdvanced: Int, emitted: Bool) -> Bool {
        segmentFrames += framesAdvanced
        framesSinceEmission = emitted ? 0 : framesSinceEmission + framesAdvanced
...
package final class StreamingSessionState: @unchecked Sendable {
    ...
    var pcmOrigin = 0
    /// Reused window scratch holding only real samples (at most `windowSampleCount`).
    var window: [Float]

    /// Diagnostic mode: chunk the encoder but concatenate its outputs and decode once at
    /// the end. Ports NeMo's `simulated` flag (`speech_to_text_streaming_infer_rnnt.py:169`,
    /// `:480-490`, `:529-554`). Comparing simulated against streaming isolates
    /// decoder-state-carry bugs from encoder-context-truncation error, without needing a
    /// WER judgement call — they should agree exactly.
    let simulated: Bool
...
extension SpeechRecognitionModel {
    ...
    public func startStream(
        ...
        // time axis is symbolic, and streaming is the worst case for that: each ramp-up
        // hop hands the encoder only its real samples, so a dynamic graph sees a *new*
        // shape every hop and MPSGraph recompiles per shape until the GPU runs out of
        // memory. The f32 dynamic encoder is also silently numerically wrong at many
        // shapes on the GPU path (internal/parity/parakeet-f32-dynamic-gpu-bug.md), so
        // there is no version of this worth attempting — and without this guard
        // `validate(encoderMelFrames: nil)` has nothing to check and waves it through.
        ...
        streaming = StreamingSessionState(
            config: config, decoder: parakeet, tdtConfig: tdtConfig, simulated: simulated,
    ...
    public func finishStream() async throws -> String {
        ...
        while try await runHopIfReady(session) {}
        if session.simulated, session.aggregatedFrames > 0 {
    ...
    private func runHopIfReady(_ session: StreamingSessionState) async throws -> Bool {
        ...
        // Hand the encoder only real audio. `fromPCM` pads up to the traced window and
        // reports the valid-frame count from the array length, so a pre-zero-padded buffer
        // would fold the padding into the per-bin normalization statistics — which is
        // ruinous on the first hops, where real audio is a small fraction of the window.
        // Tail padding then also keeps `attention_mask` a *prefix*, the only shape HF's
        // mask and the subsampling channel mask can express.
        ...
        let (encOut, encShape, validEnc) = try await runEncoder(pcm: session.window)

        // Exact per-chunk accounting — never the `validEncoderFrames` heuristic, whose
        // round-to-nearest boundary-frame drop would lose a real frame every hop.
        ...
        if lower < upper, session.simulated {
            ...
            let shouldEndpoint = session.endpoint.observe(
                framesAdvanced: upper - lower, emitted: !tokens.isEmpty)
            _ = stats

            if shouldEndpoint, !session.segmentTokens.isEmpty {
                _ = try emit(session, final: true)
                // Always safe here: both endpoint conditions land on a pause, so no word
                // is cut. Resetting mid-word measured as ~48 encoder frames (3.8 s) of
                // dropped audio while the LSTM resynced from SOS.
                session.stream.resetSegment()
